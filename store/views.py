...
from .utils import get_attr, get_model_name, get_model_slug, get_model_id
...
def dashboard(request):
    ...
    if DataAdapter.use_erpnext():
        products = DataAdapter.get_products(limit=1000)
        total_products = len(products)
        categories = DataAdapter.get_categories()
    ...
    if request.user.is_staff:
        ...
        for category in categories:
            ...
            cat_name = category.get('name') if isinstance(category, dict) else category.name
            ...
            if not DataAdapter.use_erpnext() and isinstance(category, Category):
                ...
                # For ERPNext, just use placeholder data
...
def product_list(request):
    ...
    products = DataAdapter.get_products(limit=100)
    featured_products = DataAdapter.get_products(featured=True, limit=20)
    categories = DataAdapter.get_categories()
...
def product_detail(request, product_slug):
    from .data_adapter import DataAdapter
    from .utils import get_category_slug
    
    product = DataAdapter.get_product(product_slug)
    if not product:
    ...
    if DataAdapter.use_erpnext():
        # For ERPNext, get products from same category if available
    ...
    if request.user.is_authenticated and not DataAdapter.use_erpnext():
        # Wishlist only works with local Django models for now
        product_obj = product if not isinstance(product, dict) else Product.objects.filter(slug=product_slug).first()
        if product_obj:
            is_in_wishlist = Wishlist.objects.filter(user=request.user, product=product_obj).exists()
    
    # Get category slug and name for breadcrumb
    category_slug = get_category_slug(product)
    if isinstance(product, dict):
    ...
    qr_code_data = None
    if not DataAdapter.use_erpnext() and not isinstance(product, dict):
        # Only generate QR code for local Django products
        qr_code_data = product.generate_qr_code(request)
...
def category_detail(request, category_slug):
    ...
    if DataAdapter.use_erpnext():
        # For ERPNext, convert slug back to category name for filtering
        # ERPNext uses category names, not slugs
        category_name = category_slug.replace('-', ' ').title()
        
        # Get products by category name
        products = DataAdapter.get_products(category=category_name)
        
        # Try to find category info
        categories = DataAdapter.get_categories()
        category = next((c for c in categories if (c.get('slug') if isinstance(c, dict) else c.slug) == category_slug), None)
        
        # If not found by slug, create a temporary category object
        if not category:
            category = {
                'id': category_slug,
                'name': category_name,
...
def add_to_cart(request, product_slug):
    ...
    # Get product from appropriate source
    if DataAdapter.use_erpnext():
        product_data = DataAdapter.get_product(product_slug)
        if not product_data:
...
# QR Code add to cart view
def qr_add_to_cart(request, product_slug):
    """Handle QR code scan - automatically add product to cart"""
    product = get_object_or_404(Product, slug=product_slug)
    ...
    if not created:
        ...
        cart_item.save()
        messages.success(request, f'Added another {product.name} to your cart! (Quantity: {cart_item.quantity})')
    else:
        messages.success(request, f'{product.name} has been added to your cart via QR code!')
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        # Create order using DataAdapter
        from .data_adapter import DataAdapter
        ...
        order = DataAdapter.create_order(
...
def search_results(request):
    ...
    if DataAdapter.use_erpnext():
        # For ERPNext, use the service layer
        products = DataAdapter.get_products(
            category=selected_category if selected_category else None,
            featured=True if featured == 'true' else None,
            limit=100
        )
        
        # Apply client-side filtering for ERPNext results
        if query:
            products = [p for p in products if 
                       query.lower() in (p.get('name', '') if isinstance(p, dict) else p.name).lower() or
                       query.lower() in (p.get('description', '') if isinstance(p, dict) else p.description).lower()]
        
        if min_price:
            try:
                min_p = float(min_price)
                products = [p for p in products if float(p.get('price', 0) if isinstance(p, dict) else p.price) >= min_p]
            except ValueError:
                pass
        ...
        if max_price:
            try:
                ...
                products = [p for p in products if float(p.get('price', 0) if isinstance(p, dict) else p.price) <= max_p]
            except ValueError:
                pass
        
        if in_stock == 'true':
            products = [p for p in products if (p.get('stock', 0) if isinstance(p, dict) else p.stock) > 0]
        
        # Sort products
        if sort_by == 'price_asc':
            products = sorted(products, key=lambda x: float(x.get('price', 0) if isinstance(x, dict) else x.price))
        elif sort_by == 'price_desc':
            products = sorted(products, key=lambda x: float(x.get('price', 0) if isinstance(x, dict) else x.price), reverse=True)
        elif sort_by == 'newest':
            # ERPNext products don't have created_at, so skip this sort
            pass
    else:
        # Start with base queryset for Django
        if query:
...
def home(request):
    ...
    # Use DataAdapter to get data from ERPNext or Django models
    featured_products = DataAdapter.get_products(featured=True, limit=8)
    
    # Get other product lists based on data source
    if DataAdapter.use_erpnext():
        # For ERPNext, get all products and slice them
        all_products = DataAdapter.get_products(limit=50)
        new_products = all_products[:8] if all_products else []
        best_sellers = all_products[8:16] if len(all_products) > 8 else []
        on_sale = all_products[16:24] if len(all_products) > 16 else []
    else:
    ...
    if hasattr(categories, '__getitem__'):
...
def new_arrivals(request):
    ...
    if DataAdapter.use_erpnext():
        # ERPNext doesn't have new arrivals, show all products
        products = DataAdapter.get_products(limit=50)
    else:
...
def featured_products(request):
    ...
    products = DataAdapter.get_products(featured=True, limit=50)
...
def best_sellers(request):
    ...
    if DataAdapter.use_erpnext():
        # ERPNext doesn't have best sellers, show featured products
        products = DataAdapter.get_products(featured=True, limit=50)
    else:
...
def on_sale_products(request):
    ...
    if DataAdapter.use_erpnext():
        # ERPNext doesn't have sale flag, show all products
        products = DataAdapter.get_products(limit=50)
    else:
