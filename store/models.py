...
class Product(models.Model):
    ...
    def get_qr_code_url(self, base_url=None):
        ...
        if base_url is None:
            # Fallback for when base_url is not provided
            base_url = "https://example.com"
        # Ensure base_url doesn't end with /
        base_url = base_url.rstrip('/')
        return f"{base_url}/qr-add-to-cart/{self.slug}/"
    
    def generate_qr_code(self, request=None):
        """Generate QR code for this product as base64 encoded image"""
        # Build absolute URL using request if provided
        if request:
            from django.urls import reverse
            qr_url = request.build_absolute_uri(
                reverse('qr_add_to_cart', kwargs={'product_slug': self.slug})
            )
        else:
